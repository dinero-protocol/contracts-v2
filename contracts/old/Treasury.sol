...
contract REDACTEDTreasury is Ownable {
    ...
    uint256 public sBTRFLYQueue; // Delays change to sBTRFLY address
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sBTRFLY).balanceOf(msg.sender); // Can only borrow against sBTRFLY held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithBTRFLY(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");
    ...
    function setFloor(address _token, uint256 _value) external onlyManager {
        require(
            nativeFloorPrice[_token] == 0,
            "Token floor value can only be set once"
        );
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SBTRFLY) {
            // 9
