...
import {
  ...
  updateMint,
  vaultIsMariposa,
} from './Mariposa/mariposa-test-cases';
import {
  impersonateSigner,
  setupMariposa,
  setupBTRFLY,
} from './Mariposa/mockMariposa';

describe('MariposaTest', () => {
  ...
  describe('Tests for Mariposa', async () => {
    before(async () => {
      ...
      BTRFLY = await setupBTRFLY(btrfly, multisigSigner);
    ...
    it('Tries minting BTRFLY tokens with non-Mariposa contract as signer', async function () {
      await notMintBTRFLY(BTRFLY, wallet_addr, multisigSigner, txnAmt);
    });

    it('Should mint BTRFLY tokens with Mariposa set as the signer', async function () {
      await mintBTRFLY(BTRFLY, mariposaSigner, wallet_addr, txnAmt);
    });

    it("Tries setting a department address for a department that doesn't exist", async function () {
    ...
    it('Tries calling distribute on a department in the same epoch', async function () {
      await sameEpoch(Mariposa);
    });

    it('Checks we are calling the distribute function once per epoch', async function () {
    ...
    it("Should update the mint rate of an existing department and ensure we don't exceed the cap", async function () {
      await updateMint(Mariposa, multisigSigner);
    });
  });
});
