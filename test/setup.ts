...
import { getCurrentTime, impersonateAddressAndReturnSigner, toBN } from './helpers';
...
before(async function () {
  ...
  const quarters = [currentTime, currentTime + 300 * 86400, currentTime + 2 * 300 * 86400];
  const tokensUnlocking = [parseUnits('10000', 9), parseUnits('20000', 9), parseUnits('30000', 9)];
  const vesting = await (
    await ethers.getContractFactory('Vesting')
  ).deploy(mariposa.address, ownerships, basisPoints, quarters, tokensUnlocking);
